...
public class IdPMetadataParserTest {
    ...
    public void testParseFileXML() throws Exception {
        ...
        assertEquals("https://example.onelogin.com/trust/saml2/http-redirect/sso/645460",
        ...
        assertEquals("urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect",
                idpInfo.get(SettingsBuilder.IDP_SINGLE_SIGN_ON_SERVICE_BINDING_PROPERTY_KEY));
        assertEquals("https://example.onelogin.com/trust/saml2/http-redirect/slo/645460",
                idpInfo.get(SettingsBuilder.IDP_SINGLE_LOGOUT_SERVICE_URL_PROPERTY_KEY));
        assertEquals("urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect",
                idpInfo.get(SettingsBuilder.IDP_SINGLE_LOGOUT_SERVICE_BINDING_PROPERTY_KEY));
        ...
        assertEquals("https://app.onelogin.com/trust/saml2/http-post/sso/383123",
                idpInfo.get(SettingsBuilder.IDP_SINGLE_SIGN_ON_SERVICE_URL_PROPERTY_KEY));
        assertEquals("urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect",
                idpInfo.get(SettingsBuilder.IDP_SINGLE_SIGN_ON_SERVICE_BINDING_PROPERTY_KEY));
        assertEquals(Util.loadCert((String) idpInfo.get(SettingsBuilder.IDP_X509CERT_PROPERTY_KEY)), Util.loadCert(
    ...
    public void testParseXML() throws Exception {
        Document xmlDocument =
                Util.parseXML(new InputSource(getClass().getClassLoader().getResourceAsStream("data/metadata/onelogin_metadata.xml")));
    ...
    public void testParseFileXmlDesiredBindings() throws Exception {
        ...
        assertEquals("https://idp.testshib.org/idp/profile/SAML2/Redirect/SSO",
                idpInfo.get(SettingsBuilder.IDP_SINGLE_SIGN_ON_SERVICE_URL_PROPERTY_KEY));
        assertEquals("urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect",
                idpInfo.get(SettingsBuilder.IDP_SINGLE_SIGN_ON_SERVICE_BINDING_PROPERTY_KEY));
        assertEquals(Util.loadCert((String) idpInfo.get(SettingsBuilder.IDP_X509CERT_PROPERTY_KEY)), Util.loadCert(
        ...
        idpInfo = IdPMetadataParser.parseFileXML("data/metadata/idp/testshib-providers.xml", null, null, Constants.BINDING_HTTP_REDIRECT,
        ...
        idpInfo = IdPMetadataParser.parseFileXML("data/metadata/idp/testshib-providers.xml", null, null, Constants.BINDING_HTTP_ARTIFACT,
                Constants.BINDING_HTTP_ARTIFACT);
        assertEquals("https://idp.testshib.org/idp/shibboleth", idpInfo.get(SettingsBuilder.IDP_ENTITYID_PROPERTY_KEY));
        assertEquals("https://idp.testshib.org/idp/profile/Shibboleth/SSO",
                idpInfo.get(SettingsBuilder.IDP_SINGLE_SIGN_ON_SERVICE_URL_PROPERTY_KEY));
        assertEquals("urn:mace:shibboleth:1.0:profiles:AuthnRequest",
                idpInfo.get(SettingsBuilder.IDP_SINGLE_SIGN_ON_SERVICE_BINDING_PROPERTY_KEY));
        assertEquals(Util.loadCert((String) idpInfo.get(SettingsBuilder.IDP_X509CERT_PROPERTY_KEY)), Util.loadCert(
                ""));
        assertEquals("urn:mace:shibboleth:1.0:nameIdentifier", idpInfo.get(SettingsBuilder.SP_NAMEIDFORMAT_PROPERTY_KEY));
    ...
    public void testParseRemoteXML() throws Exception {
        ...
        assertEquals("https://app.onelogin.com/saml/metadata/645460", idpInfo.get(SettingsBuilder.IDP_ENTITYID_PROPERTY_KEY));
        assertEquals("https://sgarcia-us-preprod.onelogin.com/trust/saml2/http-redirect/sso/645460",
                idpInfo.get(SettingsBuilder.IDP_SINGLE_SIGN_ON_SERVICE_URL_PROPERTY_KEY));
        assertEquals("urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect",
                idpInfo.get(SettingsBuilder.IDP_SINGLE_SIGN_ON_SERVICE_BINDING_PROPERTY_KEY));
        assertEquals("https://sgarcia-us-preprod.onelogin.com/trust/saml2/http-redirect/slo/645460",
                idpInfo.get(SettingsBuilder.IDP_SINGLE_LOGOUT_SERVICE_URL_PROPERTY_KEY));
        assertEquals("urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect",
                idpInfo.get(SettingsBuilder.IDP_SINGLE_LOGOUT_SERVICE_BINDING_PROPERTY_KEY));
        assertEquals(Util.loadCert((String) idpInfo.get(SettingsBuilder.IDP_X509CERT_PROPERTY_KEY)), Util.loadCert(
        ...
        assertEquals("https://app.onelogin.com/saml/metadata/383123", idpInfo.get(SettingsBuilder.IDP_ENTITYID_PROPERTY_KEY));
        assertEquals("https://try-demo.onelogin.com/trust/saml2/http-redirect/sso/383123",
                idpInfo.get(SettingsBuilder.IDP_SINGLE_SIGN_ON_SERVICE_URL_PROPERTY_KEY));
        assertEquals("urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect",
                idpInfo.get(SettingsBuilder.IDP_SINGLE_SIGN_ON_SERVICE_BINDING_PROPERTY_KEY));
        assertEquals("https://try-demo.onelogin.com/trust/saml2/http-redirect/slo/383123",
                idpInfo.get(SettingsBuilder.IDP_SINGLE_LOGOUT_SERVICE_URL_PROPERTY_KEY));
        assertEquals("urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect",
                idpInfo.get(SettingsBuilder.IDP_SINGLE_LOGOUT_SERVICE_BINDING_PROPERTY_KEY));
        assertEquals(Util.loadCert((String) idpInfo.get(SettingsBuilder.IDP_X509CERT_PROPERTY_KEY)), Util.loadCert(
    ...
    public void testParseMultiSameSigningAndEncryptCert() throws Exception {
        ...
        assertEquals("https://app.onelogin.com/trust/saml2/http-post/sso/383123",
                idpInfo2.get(SettingsBuilder.IDP_SINGLE_SIGN_ON_SERVICE_URL_PROPERTY_KEY));
        assertEquals("urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect",
                idpInfo2.get(SettingsBuilder.IDP_SINGLE_SIGN_ON_SERVICE_BINDING_PROPERTY_KEY));
    ...
    public void testParseSeparateSingleLogoutServiceResponseLocation() throws Exception {
        ...
        assertEquals("urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect",
                idpInfo.get(SettingsBuilder.IDP_SINGLE_SIGN_ON_SERVICE_BINDING_PROPERTY_KEY));
        assertEquals("https://idp.examle.com/saml/slo", idpInfo.get(SettingsBuilder.IDP_SINGLE_LOGOUT_SERVICE_URL_PROPERTY_KEY));
        assertEquals("https://idp.examle.com/saml/sloresp",
                idpInfo.get(SettingsBuilder.IDP_SINGLE_LOGOUT_SERVICE_RESPONSE_URL_PROPERTY_KEY));
        assertEquals("urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect",
                idpInfo.get(SettingsBuilder.IDP_SINGLE_LOGOUT_SERVICE_BINDING_PROPERTY_KEY));
    }
