		...
		try {
			...
			if (settings.isStrict()) {
				...
				if (settings.getWantXMLValidation()) {
					if (!Util.validateXML(logoutRequestDocument, SchemaFactory.SAML_SCHEMA_PROTOCOL_2_0)) {
						throw new ValidationException("Invalid SAML Logout Request. Not match the saml-schema-protocol-2.0.xsd", ValidationException.INVALID_XML_FORMAT);
					}
				}
				...
				if (rootElement.hasAttribute("NotOnOrAfter")) {
					...
					if (Util.isEqualNow(notOnOrAfterDate) || Util.isBeforeNow(notOnOrAfterDate)) {
						throw new ValidationException("Could not validate timestamp: expired. Check system clock.", ValidationException.RESPONSE_EXPIRED);
					}
				}
				...
				if (rootElement.hasAttribute("Destination")) {
					...
					if (destinationUrl != null) {
						if (!destinationUrl.isEmpty() && !destinationUrl.equals(currentUrl)) {
							throw new ValidationException("The LogoutRequest was received at " + currentUrl + " instead of "
                ...
                if (settings.getWantMessagesSigned() && (signature == null || signature.isEmpty())) {
                    throw new ValidationException("The Message of the Logout Request is not signed and the SP requires it", ValidationException.NO_SIGNED_MESSAGE);
                }
			}
			...
			if (signature != null && !signature.isEmpty()) {
				...
				if (!Util.validateBinarySignature(signedQuery, Util.base64decoder(signature), certList, signAlg)) {
					throw new ValidationException("Signature validation failed. Logout Request rejected", ValidationException.INVALID_SIGNATURE);
				}
			}
