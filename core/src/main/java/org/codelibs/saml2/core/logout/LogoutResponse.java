		...
		try {
			...
			if (settings.isStrict()) {
				...
				if (settings.getWantXMLValidation()) {
					if (!Util.validateXML(this.logoutResponseDocument, SchemaFactory.SAML_SCHEMA_PROTOCOL_2_0)) {
						throw new ValidationException("Invalid SAML Logout Response. Not match the saml-schema-protocol-2.0.xsd", ValidationException.INVALID_XML_FORMAT);
					}
				}
				...
				if (requestId != null && !Objects.equals(responseInResponseTo, requestId)) {
						throw new ValidationException("The InResponseTo of the Logout Response: " + responseInResponseTo
								+ ", does not match the ID of the Logout request sent by the SP: " + requestId, ValidationException.WRONG_INRESPONSETO);
				}
				...
				if (rootElement.hasAttribute("Destination")) {
					...
					if (destinationUrl != null) {
						if (!destinationUrl.isEmpty() && !destinationUrl.equals(currentUrl)) {
							throw new ValidationException("The LogoutResponse was received at " + currentUrl + " instead of "
									+ destinationUrl, ValidationException.WRONG_DESTINATION);
						}
					}
				}

                if (settings.getWantMessagesSigned() && (signature == null || signature.isEmpty())) {
                    throw new ValidationException("The Message of the Logout Response is not signed and the SP requires it", ValidationException.NO_SIGNED_MESSAGE);
                }
			}
			...
			if (signature != null && !signature.isEmpty()) {
				...
				if (!Util.validateBinarySignature(signedQuery, Util.base64decoder(signature), certList, signAlg)) {
					throw new ValidationException("Signature validation failed. Logout Response rejected", ValidationException.INVALID_SIGNATURE);
				}
			}
